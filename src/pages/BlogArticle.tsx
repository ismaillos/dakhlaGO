    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-b border-[#C4A882]/20">
      <div className="max-w-[1200px] mx-auto px-5 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <Link to="/blog" className="text-[#4A3728]/70 text-sm font-medium hover:text-[#E8732F] transition-colors">Blog</Link>
    ...
    <div className="min-h-screen bg-white text-[#2D1F0A]">
      ...
      <section className="px-5 pb-8">
        <div className="max-w-[800px] mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[12px] text-[#8B5E34]">{article.readTime} de lecture</span>
          </div>
          <h1 className="text-[clamp(24px,4vw,40px)] font-extrabold mb-4 leading-tight font-serif text-[#2D1F0A]">{article.title}</h1>
          <p className="text-[#8B5E34] text-sm mb-6" dir="rtl">مقال Dakhla Artisanal</p>
        </div>
      </section>
      ...
      <section className="px-5 pb-16">
        <div className="max-w-[800px] mx-auto">
              ...
              if (paragraph.startsWith('## Commander')) {
                return (
                  <div key={i} className="my-8 rounded-2xl p-6 text-center border border-[#C4A882]/30 bg-[#FAF6EF]">
                    ...
                    <p className="text-[#4A3728]/50 text-xs mt-3">Livraison partout au Maroc · Paiement à la livraison</p>
                  </div>
                );
              }
          ...
          <div className="mt-12 rounded-2xl p-8 text-center border border-[#C4A882]/30 bg-[#FAF6EF]">
            <h3 className="text-xl font-bold mb-3 font-serif text-[#2D1F0A]">Commander ce produit</h3>
            <p className="text-[#4A3728]/60 text-sm mb-6">Livraison partout au Maroc · Paiement à la livraison · Emballage discret</p>
            <button
              onClick={() => navigate(article.ctaUrl)}
            ...
            <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#25D366] text-white px-6 py-3 rounded-full text-sm font-bold hover:opacity-90 transition-all shadow-[0_4px_16px_rgba(0,0,0,0.15)]">WhatsApp</a>
          </div>
        </div>
      </section>
      ...
      {related.length > 0 && (
        <section className="py-16 px-5 bg-[#FAF6EF]">
          <div className="max-w-[800px] mx-auto">
            <h2 className="text-xl font-bold mb-6 font-serif text-[#2D1F0A]">Articles <em className="text-[#E8732F] not-italic">similaires</em></h2>
